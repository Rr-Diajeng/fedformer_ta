...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.set_type == 0:       # train
            df_sel = df_raw[df_raw['month'].isin([4,5,6])]

        elif self.set_type == 1:     # val
            df_temp = df_raw[df_raw['month'].isin([6,7])]
            val_start_idx = df_temp[df_temp['month'] == 7].index[0]
            start_idx = val_start_idx - self.seq_len
            df_sel = df_temp.loc[start_idx:]

        else:                        # test
            df_temp = df_raw[df_raw['month'].isin([7,8])]
            test_start_idx = df_temp[df_temp['month'] == 8].index[0]
        ...
        self.data_y     = data
        self.data_stamp = data_stamp
        self.feature_names = feature_cols + [self.target]
        self.date_array    = df_sel['date'].values
